...
class Pelicula {
    ...
    validarArreglo(propiedad, valor){
        ...
        if(!(valor instanceof Array)) return console.error(`${propiedad} "${valor}" ingresado, NO es un arrego`);
    ...
    validarIMDB(id){
        ...
        if(!(/^([a-z]){2}([0-9]){7}$/.test(id)))
            return console.error(`IMDB id "${id}" no es valido, debe tener 9 caracteres, los 2 primeros letras minúsculas, los 7 restantes numeros`)
    }
    ...
    validarEstreno(estreno){
        ...
        if(!(/^([0-9]){4}$/.test(estreno)))
            return console.error(`Año de Estreno "${estreno}" no es valido, debe ser un numero de 4 cifras`)
    }
